...
The infection between age groups is nearly normal distributed. This means every age group may have similar possibility to get infected.
...
```{r fata-gender-percent, echo=FALSE, results="hide", fig.align='center'}
...
```{r tree, echo=FALSE, results="hide", fig.align='center'}
...
train <- select(train, Result, `Age Group`, `Currently Hospitalized`, `Currently in ICU`, `Currently Intubated`, `Ever Hospitalized`, `Ever in ICU`, `Ever Intubated`, `Neighbourhood Name`, `FSA`)

#Testing Dataset
test <- covdat1[!rownames(covdat1) %in% rownames(train),]
result <- test$Result
test <- select(test,`Age Group`, `Currently Hospitalized`, `Currently in ICU`, `Currently Intubated`, `Ever Hospitalized`, `Ever in ICU`, `Ever Intubated`, `Neighbourhood Name`, `FSA`)
...
#Fitting Model
tree <- rpart(Result~`Age Group` + `Currently Hospitalized` + `Currently in ICU` + `Currently Intubated` + `Ever Hospitalized` + `Ever in ICU` + `Ever Intubated` + `Neighbourhood Name` + `FSA`, data=covdat1, method = "class")
rpart.plot(tree, main="Tree Model")
...
try1 <- test[0,]
try1[1,] <- as.list(c("20 to 29 Years", "No", "No", "No", "No", "No", "No", "Little Portugal", "M6K"))
try1

try2 <- test[0,]
try2[1,] <- as.list(c("80 to 89 Years", "No", "No", "No", "No", "No", "No", "Little Portugal", "M6K"))

```

<span style="color:green">Here we can try to create cases to see their outcomes.</span>\
\
The first case is in "20 to 29 years" age group. And not currently in ICU, Hospitalized, Intubated.\
Also not ever in ICU, Hospitalized, Intubated.\
